...
public class FoodArticleService {
    ...
    public Object getLikeFood(int ybid){
        List<Integer> foodIds = foodLikeRepository.findFoodidByUseridOrderByCreatetime(ybid);
        return foodArticleRepository.findByIdInAndState(foodIds,0);
    }
    //获取用户收藏的菜品
    public Object getCollectionFood(int ybid){
        List<Integer> foodIds = foodCollectionRepository.findFoodidByUseridOrderByCreatetime(ybid);
        return foodArticleRepository.findByIdInAndState(foodIds,0);
    }
    //获取用户评论过的菜品
    public Object getReviewFood(int ybid){
        HashSet<Integer> set = foodReviewRepository.findFoodidByUseridAndIsdelete(ybid,0);
        List<Integer> foodIds = new ArrayList<Integer>(set);
        return foodArticleRepository.findByIdInAndState(foodIds,0);
    }
    ...
    //标签搜索，返回分页搜索的情况
    public Object titleSearch(String detail,int pageSize,int page){

        Pageable pageable = new PageRequest(page,pageSize);
        String kind = null;
        String[] kinds = detail.split(",");
        for(int i = 0; i<kinds.length;i++){
            kind = kind+"%"+kinds[i]+"%";
        }
        return foodArticleRepository.findByStateAndKindLikeOrderByCreatetimeDesc(0,kind,pageable);
    }
    //菜品名搜索，返回分页搜索的情况
    public Object nameSearch(String name,int pageSize,int page){

        Pageable pageable = new PageRequest(page,pageSize);
        return foodArticleRepository.findByStateAndKindLikeOrderByCreatetimeDesc(0,name,pageable);
    }
}